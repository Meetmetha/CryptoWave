...
export default class RPC {
  ...
  async resetSpendingLimit(amount, nfcwalletContractAddress) {
    const web3 = new Web3(this.provider);
    const transformedAmount = amount * 10 ** 8
    const fromAddress = (await web3.eth.getAccounts())[0];

    var contract = new web3.eth.Contract(nfcAbi, nfcwalletContractAddress);
    try {
      const transaction = await contract.methods.resetSpendLimit(
        transformedAmount)
      const data = transaction.encodeABI();
      console.log(data);
      const receipt = await web3.eth.sendTransaction({
  ...
  async createBond(tokenAddress, amount, decimal, hash, bondsContractAddress) {
    ...
    try {
      ...
      localStorage.setItem("currentReceipt", receipt);
      return receipt;
    } catch (error) {
      console.log(error);
    }
  ...
  async redeemGaslessBond(tokenId, secret, bondsContractAddress, biconomyKey) {
    try {
      console.log(biconomyKey);
      ...
      await contractInstance.methods.redeemBond(
        tokenId,
        secret
      ).send({
        from: address,
